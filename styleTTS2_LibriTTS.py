...
def load_model():
    global model, device, global_phonemizer, text_aligner, pitch_extractor, plbert, sampler,model_params
...
###`alpha` and `beta` is the factor to determine much we 
#use the style sampled based on the text instead of the reference. 
#The higher the value of `alpha` and `beta`, the more suitable the style it 
#is to the text but less similar to the reference.

####



def tts_LibriTTS(passage, path):

    load_model()  # Load the model and its dependencies
